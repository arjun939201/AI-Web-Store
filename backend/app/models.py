...
class User(Base):
 __tablename__="users"; id=Column(Integer,primary_key=True); email=Column(String(320),unique=True,nullable=False); name=Column(String(120)); created_at=Column(DateTime(timezone=True),default=now,nullable=False); apps=relationship("App",back_populates="creator")
class App(Base):
