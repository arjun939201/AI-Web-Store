...
def unique_slug(db,v):
 ...
 return slug
def create_app(db,spec,creator_id=None):
 app=App(slug=unique_slug(db,spec.name),name=spec.name,description=spec.description,icon=spec.icon,category=spec.category,specification=spec.model_dump(),creator_id=creator_id)
 db.add(app); db.flush()
